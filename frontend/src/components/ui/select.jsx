...
const SelectTrigger = React.forwardRef(({ className, children, ...props }, ref) => {
  ...
  return (
    <button
      ...
      type="button"
      className={cn(
        "flex h-9 w-full items-center justify-between rounded-lg border border-input bg-background px-3 py-2 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-[3px] focus:ring-primary/20 focus:border-primary/50 transition-[border-color,box-shadow] duration-150 disabled:cursor-not-allowed disabled:opacity-50",
        className
      )}
...
const SelectContent = React.forwardRef(({ className, children, ...props }, ref) => {
  ...
  const content = (
    <div
      ...
      data-select-content
      className={cn(
        "fixed z-[9999] min-w-[8rem] overflow-hidden rounded-[0.625rem] border border-border bg-popover text-popover-foreground shadow-[0_4px_16px_-4px_rgba(0,0,0,0.12),0_1px_4px_-1px_rgba(0,0,0,0.06)] max-h-[300px] overflow-y-auto",
        className
      )}
      style={{
        top: `${position.top}px`,
...
const SelectItem = React.forwardRef(({ className, children, value, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 px-2 text-sm outline-none focus:bg-muted data-[disabled]:pointer-events-none data-[disabled]:opacity-50 hover:bg-muted",
        className
      )}
